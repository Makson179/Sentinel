...
def test_doctor_result_format_is_readable() -> None:
    ...
    assert format_result(DoctorResult("warn", "Update available: abc1234")) == "[WARN] Update available: abc1234"
...
def test_doctor_collects_required_checks_with_update_warning(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "Sentinel package: sentinel 0.1.0" in messages
    assert "Sentinel executable: /usr/bin/sentinel" in messages
    assert "Sentinel install mode: pipx" in messages
    assert "Sentinel install source: https://github.com/Makson179/Sentinel.git (default branch)" in messages
    assert "Sentinel commit: aaaaaaa" in messages
    assert "Update available: bbbbbbb" in messages
...
def test_doctor_reports_dependent_codex_checks_when_codex_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda install_info: update_check.UpdateStatus(
            ...
            install_info,
            warning="could not determine installed Sentinel commit",
        ),
    )
