...
def code_context_prompt_guidance(mode: str) -> str:
    ...
    return (
        ...
        "page through supported source files with shell reads. Recover inside Code Context "
        "before falling back: follow cursors until the needed page is returned, resolve an "
        "ambiguous symbol with list_symbols and an exact selector, and correct invalid "
        "arguments. Native shell/file reads are allowed only after a terminal Code Context "
        "capability limit remains (for example an unsupported format, an unavoidably "
        "oversized result, or service unavailability after retry), or when the operation is "
        "not representable by these tools. Repo-wide free-text search for literals, config "
        "keys, or documentation text with rg is allowed; after it identifies a target code "
        "file or symbol, return to Code Context. Directory listings, Git state, command "
        "execution, and generated program output are also outside this read policy; state "
        "the concrete fallback reason when falling back. find_references is syntactic "
