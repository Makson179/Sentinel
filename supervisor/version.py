...
def probe_codex(executable: str = "codex") -> CapabilityReport:
    ...
    _, version = run_probe([executable, "--version"])
    _, help_text = run_probe([executable, "exec", "--help"])
    missing = []
    if "exec" not in help_text and "Usage" not in help_text:
    ...
    if "--json" not in help_text and "--experimental-json" not in help_text:
