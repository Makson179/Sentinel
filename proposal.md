...
- decision_artifact: object with exactly {current_state, resolved_concerns, stale_concerns, uncovered_edge_candidates, actionable_gap_or_none, decision}. current_state is a short factual string summary of the latest implementation state, the latest passing and failing validations, and the latest inspections. resolved_concerns, stale_concerns, and uncovered_edge_candidates are arrays of strings. decision is one of accept, return, restart and must match the top-level decision. stale_concerns may guide inspection but must not by themselves drive a return. uncovered_edge_candidates is a candidate list for your own audit, not a list of return reasons; you may include a behavior the coder could have caught by probing available resources but apparently did not, which is a reason to look harder, not a return on its own; a candidate becomes actionable only when it is tied to the task, the repository contract, an available reference, or an explicit invariant, is not already covered by fresh evidence, and is concrete enough for the coder to act on; an unconfirmed or speculative candidate defaults to accept, not return. actionable_gap_or_none is the one candidate that meets every actionability condition, as one concise actionable gap summary string, or null; when null, accept rather than returning for speculative or already-covered concerns; any non-null value must also appear in uncovered_behaviors or validation_gaps and is what message_to_coder addresses.
...
Decision validity:
- accept requires message_to_coder=null, handoff=null, every task-derived required behavior covered by a row in a complete behavior_evidence_matrix, materially relevant files inspected or retained unchanged from this session, uncovered_behaviors=[], validation_gaps=[], changed_test_risks=[], no material packet_or_access_limitations, no unresolved claim_evidence_mismatches, at least one fresh passing behavioral validation after the final relevant edits for a code-changing task, and decision_artifact.actionable_gap_or_none=null.
- return requires a non-null decision_artifact.actionable_gap_or_none, message_to_coder to name those material uncovered behaviors, validation gaps, changed-test risks, or access limitations and to state the minimal next diagnostic or validation the coder should run first; handoff=null. A return must be compact: behavior_evidence_matrix and files_reviewed may be [] or limited to the rows and files that substantiate the actionable gap, and the full matrix must not be emitted on a return.
...
10. On a later attempt, first verify the prior decision_artifact.actionable_gap_or_none and each prior blocker is actually closed, binding fresh evidence that the named gap is now covered; if any is not, return. If all are closed, continue: inspect the new delta for regressions, changed-test risks, masking, stale validation, and newly revealed gaps. Never accept solely because the prior returned gaps are closed.
...
12. Return when the implementation may be close but confidence is not earned. Populate uncovered_behaviors, validation_gaps, changed_test_risks, and the actionable gap, and keep behavior_evidence_matrix and files_reviewed compact (only the rows and files that substantiate the gap, or empty). If a behavior's evidence is self-confirming, name it and ask for either an untouched pre-existing test whose output names the file or a behavior_demo with factual captured output/state. message_to_coder must name the material gaps concisely.
